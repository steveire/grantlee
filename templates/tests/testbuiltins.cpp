...
void TestBuiltinSyntax::testBasicSyntax_data()
{
  ...
  hash.clear();
  QVariantList list;
  list << QStringLiteral( "a" ) << QStringLiteral( "b" ) << QStringLiteral( "c" ) << QStringLiteral( "d" );
...
void TestBuiltinSyntax::testEnums_data()
{
  ...
  QTest::newRow( "class-enums23" )
      << QStringLiteral( "{% with var.animals as result %}{{ result.key }},{{ result }},{{ result.scope }}{% endwith %}" ) << dict
      << QStringLiteral( "Tigers,1,OtherClass" ) << NoError;
  QTest::newRow( "class-enums24" )
      << QStringLiteral( "{% with var.Animals.2 as result %}{{ result.key }},{{ result }},{{ result.scope }}{% endwith %}" ) << dict
      << QStringLiteral( "Bears,2,OtherClass" ) << NoError;
  QTest::newRow( "class-enums25" )
      << QStringLiteral( "{% with var.Bears as result %}{{ result.key }},{{ result }},{{ result.scope }}{% endwith %}" ) << dict
      << QStringLiteral( "Bears,2,OtherClass" ) << NoError;
  QTest::newRow( "class-enums26" )
      << QStringLiteral( "{% with var.Animals as result %}{{ result.0.key }},{{ result.1.key }},{{ result.2.key }}{% endwith %}" ) << dict
      << QStringLiteral( "Lions,Tigers,Bears" ) << NoError;
  ...
  QTest::newRow( "class-enums35" ) << QString::fromLatin1( "{% with var.SecondEnum as result %}"
                                        ...
                                        "{{ result }},{{ result.scope }}"
                                      "{% endwith %}" )
                                   << dict << QStringLiteral( "2,4,8,Voter,Consumer,Citizen,,SomeClass" ) << NoError;
  ...
  QTest::newRow( "qt-enums06" ) << QStringLiteral( "{{ Qt.DoesNotExist }}" ) << dict << QString() << NoError;
}
...
void TestBuiltinSyntax::testTypeAccessors_data()
{
  ...
  QTest::newRow( "type-accessors-string23" ) << QStringLiteral( "{% for line in str.splitlines %}{{ line }};{% endfor %}" ) << dict << QStringLiteral( "one;two three;four;" ) << NoError;
