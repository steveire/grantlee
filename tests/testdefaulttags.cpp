...
void TestDefaultTags::testForTag_data()
{
  ...
  list.clear();
  dict.insert( "values", list );
  QTest::newRow( "for-tag03" ) << "{% for val in values %}({{ val }} sdfsdf,){% endfor %}" << dict << "" << NoError;
  QStringList emails;
  emails << "one" << "two";
  QVariantHash obj;
  obj.insert("emails", emails);
  dict.insert("contact", obj);
  QTest::newRow( "for-tag04" ) << "{% for val in contact.emails %}({{ val }},){% endfor %}" << dict << "(one,)(two,)" << NoError;
  emails.clear();
  obj.insert("emails", emails);
  dict.insert("contact", obj);
  QTest::newRow( "for-tag05" ) << "{% for val in contact.emails %}({{ val }},){% endfor %}" << dict << "" << NoError;
  list.clear();
  list << 1 << 2 << 3;
  dict.insert( "values", list );
